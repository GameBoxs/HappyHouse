...
export default {
    ...
    methods: {
        ...
        returnDetail() {
            this.$router.replace({path:'noticedetail', query:{id:this.NoticeDetailItem.id} });
        },
        submitEdit() {
        ...
        getDetailItem() {
            ...
            .then(({data}) =>{
                ...
                this.contentText = this.NoticeDetailItem.content;
                this.titleText = this.NoticeDetailItem.title;
            })
            .catch((error) => {
                console.log(error);
